...
class XiaoXiuxianAuto(Star):
    ...
    async def _handle_captcha(self, key: str, event, raw_text: str) -> bool:
        ...
        pause_probe = getattr(guard, "is_paused", None)
        was_paused = bool(pause_probe(key)) if callable(pause_probe) else False
        ...
        if was_paused and callable(pause_probe) and not pause_probe(key):
            self._controller("auto_alchemy", key).on_captcha_resumed(key)
        return handled
