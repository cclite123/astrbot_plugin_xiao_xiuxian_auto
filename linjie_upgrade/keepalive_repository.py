...
def _safe_state_filename(account: str, group: str) -> str:
    # 同 LinjieExecutionRepository：折叠非法字符后追加账号+群组合短哈希，
    # 消除 (a_b, c) 与 (a, b_c) 的文件名冲突并杜绝路径穿越。
    safe = (
...
class LinjieKeepaliveRepository:
    ...
    def load(self, account_id: str, group_id: str) -> LinjieKeepaliveState:
        with self._lock:
            ...
            try:
                ...
                return LinjieKeepaliveState.from_dict(
                    data, account_id=str(account_id), group_id=str(group_id)
                )
    ...
    def save(self, state: LinjieKeepaliveState) -> None:
        with self._lock:
            ...
            # 唯一临时文件名 + 原子替换，避免并发保存写坏固定 .tmp 或留下半截 JSON。
